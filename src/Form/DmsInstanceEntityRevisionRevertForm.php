...
/**
 * Provides a form for reverting a DMS Instance revision.
 *
 * @ingroup dms_instance
 */
class DmsInstanceEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'dms_instance_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('DMS Instance: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('DMS Instance %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\dms_instance\Entity\DmsInstanceEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(DmsInstanceEntityInterface $revision, FormStateInterface $form_state) {
